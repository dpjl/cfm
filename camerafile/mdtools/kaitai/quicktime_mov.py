...
class QuicktimeMov(KaitaiStruct):
    ...
    class Atom(KaitaiStruct):
        ...
        def _read(self):
            ...
            elif _on == QuicktimeMov.AtomType.meta:
                self._raw_body = self._io.read_bytes(self.len)
                self.body = self._raw_body
            elif _on == QuicktimeMov.AtomType.udta:
                self._raw_body = self._io.read_bytes(self.len)
                self.body = self._raw_body
            else:
                # self._raw_body = self._io.read_bytes(self.len)
        ...
        def len(self):
            ...
            self._m_len = ((self._io.size() - 8) if self.len32 == 0 else (
                (self.len64 - 16) if self.len32 == 1 else (self.len32 - 8)))
            return self._m_len if hasattr(self, '_m_len') else None
