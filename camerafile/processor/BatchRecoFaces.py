    ...
    def arguments(self):
        ...
        for media_file in self.media_set:
            if media_file.extension in IMAGE_TYPE and media_file.metadata[FACES].binary_value is not None:
                args_list.append(
                    BatchElement((FaceRecognition.get(self.media_set).knn_clf, media_file.metadata[FACES]),
                                 media_file.relative_path))
                return args_list

    def post_task(self, result_face_metadata: Metadata, progress_bar, replace=False):
