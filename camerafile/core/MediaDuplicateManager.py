...
class MediaDuplicateManager:
    ...
    It relies on a MediaIndexManager instance to access the indexes by date/size and date/signature.
    ...
    @staticmethod
    def get_possibly_duplicates(index_manager: MediaIndexManager) -> List[MediaFile]:
        """
        ...
        for date, size_map in index_manager.date_size_map.items():
            if len(size_map) > 1:
    ...
    @staticmethod
    def duplicates_map(index_manager: MediaIndexManager) -> Dict[int, List[List[MediaFile]]]:
        """
        ...
        for date, size_map in index_manager.date_size_map.items():
            if len(size_map) == 1:
                ...
                sig_map = index_manager.date_sig_map.get(date, {})
                # if len(sig_map) == 0:
    ...
    @staticmethod
    def duplicates_info(index_manager: MediaIndexManager) -> Dict[MediaFile, tuple]:
        """
    ...
    @staticmethod
    def propagate_signature(index_manager: MediaIndexManager) -> None:
        """
                ...
                if len(media_files) > 1:
                    if MediaDuplicateManager.propagate_metadata_value(SIGNATURE, media_files):
                        for media_file in media_files:
                            index_manager.add_media_file_by_signature(media_file)

    @staticmethod
    def propagate_camera_model(index_manager: MediaIndexManager) -> None:
        """
        ...
        for date, sig_map in index_manager.date_sig_map.items():
